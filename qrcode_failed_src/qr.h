...
int qrGetErrorCode(QRCode *qr);
char *qrGetErrorInfo(QRCode *qr);
int qrAddData(QRCode *qr, const qr_byte_t *source, int size);
int qrAddData2(QRCode *qr, const qr_byte_t *source, int size, int mode);
int qrFinalize(QRCode *qr);
int qrIsFinalized(const QRCode *qr);
int qrHasData(const QRCode *qr);
QRCode *qrClone(const QRCode *qr, int *errcode);

/*
 * 構造的連接操作用関数のプロトタイプ
 */
QRStructured *qrsInit(int version, int mode, int eclevel, int masktype, int maxnum, int *errcode);
void qrsDestroy(QRStructured *st);
int qrsGetErrorCode(QRStructured *st);
char *qrsGetErrorInfo(QRStructured *st);
int qrsAddData(QRStructured *st, const qr_byte_t *source, int size);
int qrsAddData2(QRStructured *st, const qr_byte_t *source, int size, int mode);
int qrsFinalize(QRStructured *st);
int qrsIsFinalized(const QRStructured *st);
int qrsHasData(const QRStructured *st);
QRStructured *qrsClone(const QRStructured *st, int *errcode);
