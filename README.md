...
### 使用方法
+ 项目git仓库` git@121.40.169.248:/sbk_src/sbk_client.git `，使用公司现有的服务器作为私有git仓库。参考：[搭建Git服务器](https://www.liaoxuefeng.com/wiki/0013739516305929606dd18361248578c67b8067c8c017b000/00137583770360579bc4b458f044ce7afed3df579123eca000)
+ 在`/home`目录下使用` git clone git@121.40.169.248:/sbk_src/sbk_client.git `把项目克隆至树莓派上，当然需要在安装[git](https://git-scm.com/)后。
...
+ 在`/home`目录下使用`git clone https://github.com/twam/v4l2grab`克隆项目。并进行[编译安装](https://github.com/twam/v4l2grab/wiki/Installation)。并把`v4l2grab`目录内的可执行文件`v4l2grab`放入`sbk_client`目录中。
...
+ `sudo apt-get install feh`安装[feh](https://wiki.archlinux.org/index.php/Feh_(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87))。
+ 以下是对上传图片实现的说明：
> 1. 使用`tar zxvf curl-7.62.0.tar.gz`命令解压`libcurl`压缩包，当然如果有新版本可以去[官网下载页面](https://curl.haxx.se/download.html)下载。
...
+ 如测试无误，则把*shell script*添加至开机自动运行。[几种设置树莓派开机自启的方法](https://www.jianshu.com/p/1a160067d8fd)，我在使用中发现文章的**第3种方法**在实际应用中表现最好。
