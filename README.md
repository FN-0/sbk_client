...
+ 在`/home`目录下使用`git clone https://github.com/twam/v4l2grab`克隆项目。并进行[编译安装](https://github.com/twam/v4l2grab/wiki/Installation)。并把`v4l2grab`目录内的可执行文件`v4l2grab`放入`14src`目录中。
+ 分别进入`qrcode_src`目录与`qrcode_failed_src`目录，并分别执行`g++ *.cpp *.c -o qrcode`与`g++ *.cpp *.c -o qrcode_upload_failed`。
+ 把以上两个目录里编译成功的可执行文件复制到上一级目录即`14src`中。
